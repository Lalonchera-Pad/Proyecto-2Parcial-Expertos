...
def extraer_proposiciones(enunciado, proposiciones_existentes):
    ...
    for p in proposiciones:
        p = p.strip()
        if p in proposiciones_existentes:
            continue  # Ignorar si la proposición ya existe
        ...
        if "NO" in p:
            ...
            if formula:
                formula += " ∧ "  # Añadir el operador 'y' si ya hay una fórmula
            formula += f"~{var}"  # Añadir la proposición negada
        else:
            variables[var] = p
            if formula:
                formula += " ∧ "  # Añadir el operador 'y' si ya hay una fórmula
            formula += var  # Añadir la proposición
...
if opcion == '1':
    ...
    for enunciado in enunciados:
        ...
        data_to_save = {
            ...
            'tabla_estados': estado_variables(formula).to_dict(orient='index')
        }
