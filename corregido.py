...
def extraer_proposiciones(enunciado):
    ...
    for i, p in enumerate(proposiciones):
        ...
        if "NO" in p:
            p_sin_no = p.replace("NO", "").strip()  # Eliminar "NO" para obtener solo la proposición
            variables[var] = p_sin_no  # Guardar la proposición sin "NO" en variables
            formula = formula.replace(p, f"~{var}")  # Reemplazar en la fórmula
        else:
